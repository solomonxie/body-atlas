...
/// Proportioned side-view person for the first-aid scenes, built from simple shapes.
...
/// Rescuer seen from the front, kneeling behind the patient; arms locked straight down to `hands`.
struct FrontRescuer {
    var h: Double = 200
    var skin = hex("#F2C9A5")
    var line = hex("#C9A58A")
    ...
    /// `hands` = where the palms meet the patient; `spread` > 0 puts hands apart (on the shoulders)
    func draw(_ s: inout Sketch, hands: CGPoint, spread: Double = 0, drop: Double = 0) {
        let shoulderY = hands.y - 0.33 * h + drop
        let sx = 0.1 * h, waistY = hands.y - 0.02 * h
        let cx = hands.x
        s.path("M \(cx - sx) \(shoulderY) L \(cx + sx) \(shoulderY) L \(cx + 0.075 * h) \(waistY) L \(cx - 0.075 * h) \(waistY) Z",
               fill: shirt, stroke: shirtLine, lw: 1.5)
        s.rect(cx - 0.02 * h, shoulderY - 0.035 * h, 0.04 * h, 0.04 * h, fill: skin, stroke: line)
        s.ellipse(cx, shoulderY - 0.09 * h, 0.05 * h, 0.062 * h, fill: skin, stroke: line, lw: 1.5)
        s.path("M \(cx - 0.05 * h) \(shoulderY - 0.1 * h) C \(cx - 0.05 * h) \(shoulderY - 0.17 * h) \(cx + 0.05 * h) \(shoulderY - 0.17 * h) \(cx + 0.05 * h) \(shoulderY - 0.1 * h)",
               fill: hex("#6B5344"))
        for side in [-1.0, 1.0] {
            let shoulder = CGPoint(x: cx + side * (sx - 0.015 * h), y: shoulderY + 0.02 * h)
            let hand = CGPoint(x: cx + side * spread, y: hands.y - 0.012 * h)
            s.line(shoulder.x, shoulder.y, hand.x, hand.y, stroke: shirt, lw: 0.045 * h, cap: .round)
            s.line((shoulder.x + hand.x) / 2, (shoulder.y + hand.y) / 2, hand.x, hand.y, stroke: skin, lw: 0.036 * h, cap: .round)
        }
        s.ellipse(cx, hands.y - 0.005 * h, 0.045 * h + spread, 0.018 * h, fill: skin, stroke: line)
    }
}
