...
struct ViewerScreen: View {
    ...
    private var panel: some View {
        VStack(alignment: .leading, spacing: 10) {
            PillRow {
                if !history.isEmpty { Pill(label: "↶ Undo") { parts = history.removeLast() } }
                ForEach(Catalog.body.layers) { layer in
                    Pill(label: "\(layer.labelZh) \(layer.label)", selected: layers.contains(layer.id)) {
                        if layers.contains(layer.id) { layers.remove(layer.id) } else { layers.insert(layer.id) }
                    }
                }
                if parts.changedCount > 0 { Pill(label: "Reset parts (\(parts.changedCount))") { change(PartState()) } }
            }
            ...
            if isReflex, let sp = systemPoints {
                ...
                Text("Tap any part to name it — 点击任意部位查看名称. Tap an arm or leg bone/muscle to move its joint.")
