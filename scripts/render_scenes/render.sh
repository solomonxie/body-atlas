...
# Usage: scripts/render_scenes/render.sh /tmp/scenes [scenario-id…]
set -e
cd "$(dirname "$0")/../.."
...
swiftc -O -parse-as-library -o "$BIN" \
  ...
  Sources/Illustrations/Figures.swift Sources/Illustrations/Scenes/*.swift Sources/Charts/SVGPath.swift 2>&1 | grep -E "error" || true
"$BIN" "$@"
